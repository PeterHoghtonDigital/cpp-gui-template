...
- ***Linux***
    - Use the terminal command: `sudo apt install -y libc++-dev libc++abi-dev clang lld xorg-dev libx11-dev libwayland-dev wayland-protocols libxkbcommon-dev extra-cmake-modules`.
...
3) Enter this repository URL: `https://github.com/PeterHoghtonDigital/cpp-gui-template.git`
4) Choose which folder you want to download the project to and wait for it to download.
- Alternatively, if you prefer to use the terminal, you can clone via `git clone https://github.com/PeterHoghtonDigital/cpp-gui-template.git /path/to/folder`.
...
Files in `External/glad/` were generated using https://gen.glad.sh/ and target OpenGL core version 4.1 with internal loaders for APIs included.

If you are targeting a different version of OpenGL, or require extensions etc., then you can generate updated Glad files yourself using the above link and safely replace the contents of the `External/glad/` directory with the generated files. You can then reconfigure the project via **'Ctrl + Shift + P' -> 'CMake: Delete Cache and Reconfigure'**.
...
If you encounter problems or would like to suggest an improvement, please submit an [issue](https://github.com/PeterHoghtonDigital/cpp-gui-template/issues) on GitHub and I will investigate when possible.